...
- The **standalone mode** is recommended for users who want to use the code for quick inference and visualization. We provide a most easy way to install the environment by using `conda` environment file. The whole environment including `cuda` and `pytorch` can be easily installed by running the following command:
  ...
  _We install **FlashAttention** by default, yet not necessary. If FlashAttention is not available in your local environment, it's okay, check Model section in [Quick Start](#quick-start) for solution._
...
**_Let's first begin with some simple visualization demo with Sonata, our pre-trained PTv3 model:_**
...
**_Then, here are the instruction to run inference on custom data with our Sonata:_**
...
- **Model.** Load the pre-trained model by running the following command:
  ...
  If _FlashAttention_ is not available, load the pre-trained model with the following code:
